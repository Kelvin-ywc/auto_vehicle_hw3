...
class RelativePosBias(nn.Module):
    def __init__(self, group_size, num_heads):
        super().__init__()
        self.group_size = group_size
        self.num_heads = num_heads

        self.relative_position_bias_table = nn.Parameter(
            torch.zeros((2 * group_size[0] - 1) * (2 * group_size[1] - 1), num_heads))
    ...
    def flops(self, N):
        ...
        flops = N * self.group_size[0] * self.group_size[1] * self.num_heads
        return flops
...
class Attention(nn.Module):
    ...
    def __init__(self, dim, group_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.,
                 position_bias=True, use_dpb=True):
        ...
        self.use_dynamic_pos_bias = use_dpb
        self.init_rpb =False
        self.first_time_log = True
        if position_bias:
            self.pos = DynamicPosBias(self.dim // 4, self.num_heads, residual=False)
            self.rpb = RelativePosBias(self.group_size, self.num_heads)
...
class CrossFormerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, group_size=7, interval=8, lsda_flag=0,
                 ...
                 pad_type=0, use_extra_conv=True, use_cpe=False, no_mask=False, adaptive_interval=False, use_dpb=True):
        super().__init__()
        self.dim = dim
        ...
        self.attn = Attention(
            ...
            position_bias=(not use_cpe), use_dpb=use_dpb)
...
class Stage(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, group_size, interval,
                 ...
                 no_mask=False, adaptive_interval=False, use_acl=False, use_dpb=True):
...
class CrossFormer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=[4], in_chans=3, num_classes=1000,
                 ...
                 adaptive_interval=False, use_acl=False, use_dpb=True,  **kwargs):
        super().__init__()
